...
module.exports = {
  ...
  onStart: async function ({ message, event, args, api, threads }) {
    ...
    // Handle on/off
    if (action === 'on') {
      if (!hasPermission) return message.reply("❌ Only admins can enable autotime.");
      global.autotimeStatus[threadID] = true;
      return message.reply("✅ **Autotime ENABLED**\n⏰ Hourly messages will be sent in this group.");
    }

    if (action === 'off') {
      if (!hasPermission) return message.reply("❌ Only admins can disable autotime.");
      global.autotimeStatus[threadID] = false;
      return message.reply("❌ **Autotime DISABLED**\n⏰ No more hourly messages in this group.");
    }

    // Default: show status
    const status = global.autotimeStatus[threadID] ? "🟢 ON" : "🔴 OFF";
    message.reply(`⏰ **AUTOTIME STATUS**\n━━━━━━━━━━━━━━\n📌 Status: ${status}\n━━━━━━━━━━━━━━\n${hasPermission ? "📌 autotime on  - Enable\n📌 autotime off - Disable" : "⚠️ Only admins can change settings"}\n━━━━━━━━━━━━━━\n💡 Type "autotime" to check status`);
  }
};

// Auto-message system (একবার শুধু রান হবে)
if (!intervalStarted) {
