...
module.exports = {
  config: {
    ...
    longDescription: "Sends automatic messages at specific times (on/off control)",
    guide: "{p}autotime\n{p}autotime on\n{p}autotime off",
    category: "system"
  },
  ...
  onStart: async function ({ message, event, args, api }) {
    ...
    return message.reply(`⏰ AUTOTIME স্ট্যাটাস\n━━━━━━━━━━━━━━\n📌 স্ট্যাটাস: ${status}\n━━━━━━━━━━━━━━\n📌 চালু করতে: autotime on\n📌 বন্ধ করতে: autotime off`);
  },

  onEvent: async function ({ api, event }) {
    const threadID = event.threadID;
    if (threadID < 0) return;
    if (!global.autotimeStatus) return;
    if (global.autotimeStatus[threadID] !== true) return;
    ...
    const matched = messages.find(item => item.timer === formattedTime);
    if (matched) {
      api.sendMessage(matched.msg, threadID).catch(() => {});
    }
  }
};
