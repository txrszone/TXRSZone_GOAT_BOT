...
module.exports = {
  ...
  onStart: async function ({ message, event, args, usersData, api, getText, role }) {
    ...
    if (args[0] && args[0].toLowerCase() === 'off') {
      if (!existingGame || !existingGame.gameOn) {
        return api.sendMessage("❌ No active game found to end!", threadID, messageID);
      }
  ...
  onReply: async function ({ message, event, api, getText, role }) {
    ...
    const isPlayerTurn = (data.playerColor === 'white' && data.chess.turn() === 'w') || 
                         (data.playerColor === 'black' && data.chess.turn() === 'b');
    
    if (!isPlayerTurn) {
      return api.sendMessage("⏳ Please wait, AI is thinking...", threadID, messageID);
    }
...
function processAIMove(data) {
  ...
  if (aiMove) {
    ...
    const checkStatus = chess.inCheck() ? ' ⚠️ Check!' : '';
    const taunt = getAITaunt(data.aiLevel);
    const tauntText = taunt ? ` ${taunt}` : '';
...
async function displayBoard(data) {
  ...
  const isPlayerTurnNow = (data.playerColor === 'white' && data.chess.turn() === 'w') || 
                           (data.playerColor === 'black' && data.chess.turn() === 'b');
  const status = data.chess.inCheck() ? '⚠️ CHECK! ⚠️' : 
  ...
  ctx.fillText(`💡 Type 'chess off' to end game`, 400, 925);

  // Save canvas
  const cacheDir = path.join(__dirname, "cache");
  if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir, { recursive: true });

  const imagePath = path.join(cacheDir, `chess_${data.userId}_${Date.now()}.png`);
