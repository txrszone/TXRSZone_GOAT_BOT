...
module.exports = {
  ...
  run: async function ({ api, event, args }) {
    ...
    const filePath = path.join(cacheDir, `mwmeme${ext}`);

    fs.ensureDirSync(cacheDir);

    api.sendMessage(`📦 Loading random Modern Warships meme...\n🖼️ Total memes: ${totalMemes}`, event.threadID);
    ...
    request(encodeURI(chosenUrl))
      ...
      .on("error", (err) => {
        ...
        api.sendMessage("⚠️ Failed to fetch meme. Please try again later.", event.threadID);
      });
  }
};
